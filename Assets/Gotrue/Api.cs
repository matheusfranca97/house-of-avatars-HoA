...
namespace Supabase.Gotrue
{
	...
	public class Api : IGotrueApi<User, Session>
	{
		...
		public async Task<Session?> SignUpWithEmail(string email, string password, SignUpOptions? options = null)
		{
			...
			var response = await Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, endpoint, body, Headers);
		...
		{
			var body = new Dictionary<string, object> { { "email", email }, { "password", password } };
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, $"{Url}/token?grant_type=password", body, Headers);
		}
		...
		public async Task<PasswordlessSignInState> SignInWithOtp(SignInWithPasswordlessEmailOptions options)
		{
			...
			await Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, url, body, Headers);
		...
		public async Task<Session?> SignInAnonymously(SignInAnonymouslyOptions? options = null)
		{
			...
			return await Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, url, body, Headers);
		}
		...
		public Task<Session?> SignInWithIdToken(Provider provider, string idToken, string? accessToken = null, string? nonce = null, string? captchaToken = null)
		{
			...
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, $"{Url}/token?grant_type=id_token", body, Headers);
		}
		...
		private Task<SSOResponse?> SignInWithSsoInternal(Guid? providerId = null, string? domain = null, SignInWithSSOOptions? options = null)
		{
			...
			return Helpers.MakeRequest<SSOResponse>(UnityWebRequest.kHttpVerbPOST, $"{Url}/sso", body, Headers);
		}
		...
		public Task<BaseResponse> SendMagicLinkEmail(string email, SignInOptions? options = null)
		{
			...
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, endpoint, data, Headers);
		}
		...
		public Task<BaseResponse> InviteUserByEmail(string email, string jwt, InviteUserByEmailOptions? options = null)
		{
			...
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, url, body, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<Session?> SignUpWithPhone(string phone, string password, SignUpOptions? options = null)
		{
			...
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, endpoint, body, Headers);
		}
		...
		public Task<Session?> SignInWithPhone(string phone, string password)
		{
			...
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, $"{Url}/token?grant_type=password", data, Headers);
		}
		...
		public Task<BaseResponse> SendMobileOTP(string phone)
		{
			...
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, $"{Url}/otp", data, Headers);
		}
		...
		public Task<Session?> VerifyMobileOTP(string phone, string token, MobileOtpType type)
		{
			...
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, $"{Url}/verify", data, Headers);
		}
		...
		{
			var data = new Dictionary<string, string> { { "email", email } };
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, $"{Url}/recover", data, Headers);
		}
		...
		public Task<Session?> ExchangeCodeForSession(string codeVerifier, string authCode)
		{
			...
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, url.ToString(), body, Headers);
		}
		...
		public Task<MfaEnrollResponse?> Enroll(string jwt, MfaEnrollParams mfaEnrollParams)
		{
			...
			return Helpers.MakeRequest<MfaEnrollResponse>(UnityWebRequest.kHttpVerbPOST, $"{Url}/factors", body, CreateAuthedRequestHeaders(jwt));
		}

		/// <inheritdoc />
		public Task<MfaChallengeResponse?> Challenge(string jwt, MfaChallengeParams mfaChallengeParams)
		{
			return Helpers.MakeRequest<MfaChallengeResponse>(UnityWebRequest.kHttpVerbPOST, $"{Url}/factors/{mfaChallengeParams.FactorId}/challenge", null, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<MfaVerifyResponse?> Verify(string jwt, MfaVerifyParams mfaVerifyParams)
		{
			...
			return Helpers.MakeRequest<MfaVerifyResponse>(UnityWebRequest.kHttpVerbPOST, $"{Url}/factors/{mfaVerifyParams.FactorId}/verify", body, CreateAuthedRequestHeaders(jwt));
		}

		/// <inheritdoc />
		public Task<MfaUnenrollResponse?> Unenroll(string jwt, MfaUnenrollParams mfaUnenrollParams)
		{
			return Helpers.MakeRequest<MfaUnenrollResponse>(UnityWebRequest.kHttpVerbDELETE, $"{Url}/factors/{mfaUnenrollParams.FactorId}", null, CreateAuthedRequestHeaders(jwt));
		}

		/// <inheritdoc />
		public Task<BaseResponse> ListFactors(string jwt, MfaAdminListFactorsParams listFactorsParams)
		{
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbGET, $"{Url}/admin/users/{listFactorsParams.UserId}/factors", null, CreateAuthedRequestHeaders(jwt));
		}

		/// <inheritdoc />
		public Task<MfaAdminDeleteFactorResponse?> DeleteFactor(string jwt, MfaAdminDeleteFactorParams deleteFactorParams)
		{
			return Helpers.MakeRequest<MfaAdminDeleteFactorResponse>(UnityWebRequest.kHttpVerbDELETE, $"{Url}/admin/users/{deleteFactorParams.UserId}/factors/{deleteFactorParams.Id}", null, CreateAuthedRequestHeaders(jwt));
		}
		...
		public async Task<ProviderAuthState> LinkIdentity(string token, Provider provider, SignInOptions options)
		{
			...
			await Helpers.MakeRequest(UnityWebRequest.kHttpVerbGET, state.Uri.ToString(), null, CreateAuthedRequestHeaders(token));
			return state;
		}
		...
		{
			var result = await Helpers.MakeRequest(UnityWebRequest.kHttpVerbDELETE, $"{Url}/user/identities/${userIdentity.IdentityId}", null, CreateAuthedRequestHeaders(token));
		...
		public Task<BaseResponse> SignOut(string jwt, SignOutScope scope = SignOutScope.Global)
		{
			...
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, $"{Url}/logout?scope={Core.Helpers.GetMappedToAttr(scope).Mapping}", data, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<User?> GetUser(string jwt)
		{
			...
			return Helpers.MakeRequest<User>(UnityWebRequest.kHttpVerbGET, $"{Url}/user", data, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<User?> GetUserById(string jwt, string userId)
		{
			...
			return Helpers.MakeRequest<User>(UnityWebRequest.kHttpVerbGET, $"{Url}/admin/users/{userId}", data, CreateAuthedRequestHeaders(jwt));
		}
		...
		{
			return Helpers.MakeRequest<User>(UnityWebRequest.kHttpVerbPUT, $"{Url}/user", attributes, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<UserList<User>?> ListUsers(string jwt, string? filter = null, string? sortBy = null, SortOrder sortOrder = SortOrder.Descending, int? page = null, int? perPage = null)
		{
			...
			return Helpers.MakeRequest<UserList<User>>(UnityWebRequest.kHttpVerbPUT, $"{Url}/admin/users", data, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<User?> CreateUser(string jwt, AdminUserAttributes? attributes = null)
		{
			...
			return Helpers.MakeRequest<User>(UnityWebRequest.kHttpVerbPOST, $"{Url}/admin/users", attributes, CreateAuthedRequestHeaders(jwt));
		}
		...
		{
			return Helpers.MakeRequest<User>(UnityWebRequest.kHttpVerbPUT, $"{Url}/admin/users/{userId}", userData, CreateAuthedRequestHeaders(jwt));
		}
		...
		{
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPUT, $"{Url}/reauthenticate", null, CreateAuthedRequestHeaders(userJwt));
		}
		...
		{
			var data = new Dictionary<string, string>();
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbDELETE, $"{Url}/admin/users/{uid}", data, CreateAuthedRequestHeaders(jwt));
		}
		...
		{
			return Helpers.MakeRequest<Settings>(UnityWebRequest.kHttpVerbGET, $"{Url}/settings", null, Headers);
		}
		...
		public Task<BaseResponse> GenerateLink(string jwt, GenerateLinkOptions options)
		{
			...
			return Helpers.MakeRequest(UnityWebRequest.kHttpVerbPOST, url, options, CreateAuthedRequestHeaders(jwt));
		}
		...
		public Task<Session?> RefreshAccessToken(string accessToken, string refreshToken)
		{
			...
			return Helpers.MakeRequest<Session>(UnityWebRequest.kHttpVerbPOST, $"{Url}/token?grant_type=refresh_token", data, Headers.MergeLeft(headers));
		}
	}
}
